...
public class Stapler : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(Physics.BoxCast(transform.position, halfextents, Vector3.down, out RaycastHit r, transform.rotation, .1f, ~layerMask))
        {
            ...
            //Debug.Log("grounded");
            elapsed = 0;
            if (Physics.Raycast(transform.position, transform.right, 1f, ~layerMask) || !Physics.Raycast(transform.position, (transform.right + transform.up * -1).normalized, 1.2f, ~layerMask))
            {
